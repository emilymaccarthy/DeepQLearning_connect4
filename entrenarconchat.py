...
def entrenar(episodes:int=500,
    ...
    model_name:str = f"trained_model_vs_{nombre_oponente}_{episodes}_{gamma}_" + \
                     f"{epsilon_start}_{epsilon_min}_{epsilon_decay}_" + \
    ...
    agent:Agent = DeepQLearningAgent(
        ...
        device=device,
        gamma=gamma, 
        epsilon=epsilon_start,        # ✅ Epsilon inicial
        epsilon_min=epsilon_min,      # ✅ Epsilon mínimo
        epsilon_decay=epsilon_decay,  # ✅ Decaimiento
        lr=alpha, 
        batch_size=batch_size, 
        memory_size=memory_size,      # ✅ Memoria de Replay
    ...
    for episode in range(episodes):
        ...
        episode_losses = []  
        dqn_player = 1  # DQN siempre arranca como jugador 1
    
        while not done:
            valid_actions = env.available_actions()

            if env.game.current_player == dqn_player or opponent is None:  
                # Turno del DQN (o self-play si no hay oponente)
        ...
        if (episode + 1) % 100 == 0:
            ...
            if verbose: 
                print(f"Episodio {episode + 1} finalizado. Epsilon: {agent.epsilon:.4f} | Loss promedio: {avg_loss:.6f}", flush=True)
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="Entrenar un agente usando DQN en el ambiente de 'Connect4'.")
